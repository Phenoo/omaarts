...
export default function EditActivityPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!window.confirm("Are you sure you want to update this activity?")) return;
  ...
  if (loading) {
    return (
      <div className="flex flex-col gap-8 text-[var(--foreground)] min-h-[50vh] justify-center items-center">
        ...
        <span className="font-mono text-xs uppercase tracking-widest text-[var(--text-muted)]">Loading activity...</span>
      </div>
    );
  }
  ...
  if (!editingActivity) {
    return (
      <div className="flex flex-col gap-4 text-[var(--foreground)] items-center pt-10">
        <h2 className="font-serif text-2xl">Activity not found</h2>
        <Link href="/admin/activities" className="text-[var(--accent-purple)] hover:underline font-mono text-sm">
    ...
    <div className="flex flex-col gap-8 text-[var(--foreground)] max-w-4xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[var(--border-soft)] pb-6">
        <div>
          ...
          <h1 className="font-serif text-3xl md:text-4xl font-semibold">Edit Activity</h1>
        </div>
      </div>
