    ...
    <article className="experience-card group">
      <Link href={`/experiences/${experience.slug}`} className="block" aria-label={`View ${experience.name} experience`}>
        <div className="experience-card__image">
          <Image src={experience.images[0] || '/images/studio/IMG_0890.png'} alt={`${experience.name} at Artsy by Oma`} fill sizes="(max-width: 640px) 92vw, (max-width: 1024px) 45vw, 30vw" className="object-cover transition-transform duration-700 group-hover:scale-[1.03]" />
        </div>
      </Link>
      ...
      <div className="mt-4 flex items-center justify-between gap-4 border-t border-[var(--border-soft)] pt-3">
        <span className="font-sans text-sm md:text-base font-extrabold tracking-tight text-[var(--accent-orange)]">{price}</span>
        <Link href={`/experiences/${experience.slug}`} className="button button--small font-black">Book / enquire</Link>
      </div>
    </article>
  );
}
